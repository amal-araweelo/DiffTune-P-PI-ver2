...
if param1.generateVideo
    video_obj = VideoWriter('Results\DiffTune-P-PI','MPEG-4');
...
learningRate = 1;  % Calculate       0.5 kunne ikke finde bedre løsning, 0.05 så rammer den k_vec = 0.1 for alle
maxIterations = 50;
itr = 0;
...
while (1)
    ...
    for k = 1 : length(time) - 1
        ...
        [u, v] = controller(X, Xref, k_vec, theta_r_dot(k), theta_r_2dot(k), v, param, dt); 
        ...
        theta_gradient = theta_gradient + 2 * [0 0 0 Xref-X(4)] * dx_dtheta;
   ...
   % Compute the RMSE (root-mean-square error)
    ...
    text(0.25,-1.05,['k\_pos = ' num2str(k_vec(1)) ', grad = ' num2str(theta_gradient(1))]);
    text(0.25,-1.15,['k\_vel = ' num2str(k_vec(2)) ', grad = ' num2str(theta_gradient(2))]);
    text(0.25,-1.25,['k_i = ' num2str(k_vec(3)) ', grad = ' num2str(theta_gradient(3))]);
    ...
    text(25,0.7e-3,['iteration = ' num2str(length(rmse_hist))],'FontSize',12);
    xlabel('iterations');
    ylabel('RMSE [rad]');
    set(gca,'FontSize',10);
    plotedit(gca,'on');
